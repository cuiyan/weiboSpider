...
conn = sqlite3.connect('sinaweibo.sqlite')
cur = conn.cursor()

cur.execute('''DROP TABLE IF EXISTS weibo_record''')
cur.execute('''DROP TABLE IF EXISTS weibo_user''')

cur.execute('''CREATE TABLE weibo_user (userId INTEGER, userName TEXT,weiboNum INTEGER,following INTEGER,followers INTEGER)''')
cur.execute('''CREATE TABLE weibo_record (user_id INTEGER,userName TEXT,weibos INTEGER,num_zan INTEGER,num_forwarding INTEGER,num_comment INTEGER)''')

class weibo:
    #将your cookie替换成自己的cookie
    cookie = {"Cookie": "_T_WM=fcb0aeb036fc8c0ac3f6dc9d5121447a; SUB=_2A251_CB7DeRhGeBM6lUX9ifEzjyIHXVXH0AzrDV6PUJbkdBeLVCmkW0af5lmtfGAm9YysZ6AaU5ABhfqUQ..; SUHB=0Elax-KNWknyI0; SCF=AoHru9PYb8k_FV2ZN1YmgYRQ60gdEww_yszF8mTSiAhmnEPB47B020Cz1sQykiVymxmv2RS4BlCd-RRbqlKXNHA.; SSOLoginState=1492668459; M_WEIBOCN_PARAMS=luicode%3D20000174%26uicode%3D20000174"} 
    #weibo类初始化
    ...
    def writeDB(self):
        try:
            cur.execute('SELECT userId FROM weibo_user WHERE userId = ?',(self.user_id,))
            row = cur.fetchone()
            print "==========================写入数据库",row
            if row is None:
                cur.execute('INSERT INTO weibo_user(userId,userName,weiboNum,following,followers) VALUES(?,?,?,?,?)',(str(self.user_id),self.userName.decode('GB2312'),str(self.weiboNum),str(self.following),str(self.followers),))
                print "userID",str(self.user_id)
                
                for i in range(1,self.weiboNum2 + 1):
                      #text=str(i) + ':' + self.weibos[i-1] + '\n'+'点赞数：' + str(self.num_zan[i-1]) + '     转发数：' + str(self.num_forwarding[i-1]) + '     评论数：' + str(self.num_comment[i-1]) + '\n\n'
                      #result = result + text
                    cur.execute('INSERT INTO weibo_record(user_id ,userName ,weibos ,num_zan ,num_forwarding ,num_comment ) VALUES(?,?,?,?,?,?)',(str(self.user_id),self.userName.decode('GB2312'),self.weibos[i-1].decode('gbk'),str(self.num_zan[i-1]),str(self.num_forwarding[i-1]),str(self.num_comment[i-1]),))
            else:
                cur.execute('UPDATE weibo_user SET following=? ,followers=? WHERE userId=?',(self.following,self.followers,self.user_id,))
...
user_id = 6014403348 #可以改成任意合法的用户id（爬虫的微博id除外）
